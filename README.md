...
* [ ] dtype and device annotation (is actually implemented but not documented,
 and the way its annotated is a bit clumsy. Wouldn't something like ``"B H W 
 float16"`` be better than ``["B H W", None, torch.float16]``? We would just
 treat ``cpu, cuda:0, cuda:1, ..., float, float32, int, int32, bool, ...`` as
 keywords that cannot be used as a variable)
